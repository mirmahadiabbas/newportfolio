...
(function () {
  ...
  var ICONS = {
    ...
    web:    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><rect x="3" y="4" width="18" height="16" rx="2"/><path d="M3 9h18"/><path d="M9 13l-2 2 2 2M15 13l2 2-2 2"/></svg>',
    spark:  '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M12 2l1.9 5.7L20 9.5l-5.4 2.6L12 18l-2.6-5.9L4 9.5l6.1-1.8z"/><path d="M19 15l.8 2.2L22 18l-2.2.8L19 21l-.8-2.2L16 18l2.2-.8z"/></svg>',
    flow:   '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><rect x="3" y="3" width="6" height="6" rx="1"/><rect x="15" y="9" width="6" height="6" rx="1"/><rect x="3" y="15" width="6" height="6" rx="1"/><path d="M9 6h3v6h3M9 18h6v-3"/></svg>',
    bars:   '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M4 20V10M10 20V4M16 20v-8M22 20H2"/></svg>',
    film:   '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><rect x="3" y="4" width="18" height="16" rx="2"/><path d="M7 4v16M17 4v16M3 9h4M3 15h4M17 9h4M17 15h4"/><path d="M11 10l4 2-4 2z"/></svg>',
    plus:   '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><circle cx="12" cy="12" r="9"/><path d="M12 8v8M8 12h8"/></svg>'
  };
  ...
  // inclusive month count, the way CVs and LinkedIn count it:
  // Oct 2021 to Sep 2023 = 24 months = "2 yrs"
  function monthIndex(ym) {
    if (ym === 'now') { var d = new Date(); return d.getFullYear() * 12 + d.getMonth(); }
    var p = ym.split('-');
  ...
  function renderGantt(g) {
    ...
    var rows = g.rows.map(function (r, i) {
      ...
      var months = Math.max(1, monthIndex(r.to) - monthIndex(r.from) + 1);
      var dur = durText(months) + (ongoing ? ' · ongoing' : '');
      var logo = r.logo ? '<img class="g-logo" src="' + esc(r.logo) + '" alt="" loading="lazy">' : '';
      return '<div class="g-row">' +
        '<div class="g-label"><span class="g-org">' + logo + esc(r.org) + '</span>' +
        ...
        ' aria-label="' + esc(r.org + ', ' + r.sub + ', ' + dates + ' (' + dur + '). Jump to details.') + '"></button></div>' +
        '</div>';
    }).join('');
  ...
  function renderProjects() {
    ...
    return section('projects', 'section--dark',
      ...
      (p.cta ? '<p class="proj-more reveal"><a class="arrow-link" href="' + esc(p.cta.href) + '">' +
        esc(p.cta.label) + ' ' + ARROW + '</a></p>' : ''));
  }
  ...
  function renderServices() {
    ...
    var cards = s.items.map(function (it) {
      ...
      return '<div class="svc-card reveal">' +
        '<div class="svc-icon" aria-hidden="true">' + (ICONS[it.icon] || ICONS.plus) + '</div>' +
        '<h3 class="svc-title">' + esc(it.title) + '</h3>' +
        '<p class="svc-desc">' + esc(it.desc) + '</p>' +
        '<p class="svc-meta">' + meta + '</p>' +
        '</div>';
    }).join('');
    return section('services', 'section--ivory2',
      header('Services', s.title, s.intro) +
      '<div class="svc-grid">' + cards + '</div>' +
      '<div class="svc-cta reveal">' +
        '<a href="' + esc(s.cta.href) + '" class="btn btn--brass">' + esc(s.cta.label) + '</a>' +
        '<p class="svc-note">' + esc(s.cta.note) + '</p>' +
      '</div>');
  }
